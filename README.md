## Features
Our free GIF Scroll Animation actor is an automation tool to let you capture a scrolling web page as a GIF. 
...
## How it works
It's very simple to use. You give the actor a URL, it visits the web page and takes screenshots while scrolling the page itself. The screenshots are then used as frames and turned into a GIF.
...
If you need the result sooner, enable fast mode. Fast mode prioritizes speed over smoothness by taking fewer frames, scrolling in larger steps, and skipping extra compression outputs.
...
| fastMode | boolean | No | false | Prioritizes quicker results with fewer frames, larger page scroll steps, and no compressed derivative GIF outputs. |
